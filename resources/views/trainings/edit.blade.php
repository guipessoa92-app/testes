<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Editar Treino: ') . $training->name }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('trainings.update', $training) }}" method="POST">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $training->name)" required autofocus />
                        ...
                        <div class="mt-4">
                            ...
                            <div class="mt-2 grid grid-cols-2 md:grid-cols-4 gap-4">
                                @php
                                    $associatedDays = $training->daysOfWeek->pluck('id')->toArray();
                                @endphp
                                @foreach ($daysOfWeek as $day)
                                    <label class="inline-flex items-center">
                                        <input type="checkbox" name="days_of_week[]" value="{{ $day->id }}"
                                               @if(in_array($day->id, $associatedDays)) checked @endif
                        ...
                        <div class="mt-4">
                            ...
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm">{{ old('description', $training->description) }}</textarea>
